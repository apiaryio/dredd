...
},
{
  ...
  describe('when the actual response is non-empty', () => {
    ...
    it('prints four warnings for each of the responses', () => assert.equal(runtimeInfo.dredd.logging.match(/HTTP 204 and 205 responses must not include a message body/g).length, 4));
    it('prints four failures for each non-matching status code', () => assert.equal(runtimeInfo.dredd.logging.match(/fail: statusCode: Status code is '200' instead of/g).length, 4));
  ...
  describe('when the actual response is empty', () => {
    ...
    it('evaluates all the responses as invalid', () => assert.deepInclude(runtimeInfo.dredd.stats, { tests: 4, failures: 4 }));
    it('prints two warnings for each of the non-empty expectations', () => assert.equal(runtimeInfo.dredd.logging.match(/HTTP 204 and 205 responses must not include a message body/g).length, 2));
    it('prints two failures for each non-matching body (and status code)', () => assert.equal(runtimeInfo.dredd.logging.match(/fail: body: Real and expected data does not match.\nstatusCode: Status code is '200' instead of/g).length, 2));
    it('prints two failures for each non-matching status code', () => assert.equal(runtimeInfo.dredd.logging.match(/fail: statusCode: Status code is '200' instead of/g).length, 2));
  });
}));
