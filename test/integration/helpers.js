...
const runDreddWithServer = function(dredd, app, serverPort, callback) {
  ...
  return server = app.listen(serverPort, function(err, serverRuntimeInfo) {
    if (err) { return callback(err); }
...
const killAll = function(pattern, callback) {
  return ps.lookup({arguments: pattern}, function(err, processList) {
