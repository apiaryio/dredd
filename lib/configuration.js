...
function validateLogLevel(level, levels) {
  if (!levels.includes(level)) {
    throw new Error(`The logging level '${level}' is unsupported, `
...
function getLogLevel(options) {
  ...
  return loglevel && validateLogLevel(loglevel, supportedLogLevels)
    ? loglevel
    : defaultLogLevel;
}
