...
function resolveConfig(config) {
  const inConfig = R.compose(
    // Evolve deep nested properties instead of deep merge
    // to retain class instances methods (i.e. "emitter.on").
    R.evolve({
      custom: R.mergeDeepLeft(defaultConfig.custom),
    }),
    R.assoc('emitter', R.propOr(new EventEmitter(), 'emitter', config)),
    R.mergeRight(defaultConfig),
    flattenConfig
  )(config);
  ...
  return normalizeConfig(inConfig);
}
...
module.exports = {
  ...
  _coerceDeprecatedLevel: coerceDeprecatedLevelOption,
};
