 ...
 * @param {*} encoding
 */
export const getBodyAsBuffer = (body, encoding) => {
 ...
 */
export const normalizeBodyEncoding = (encoding) => {
  if (!encoding) {
    return 'utf-8'
  }
 ...
 * @param {Object} [options.logger] Custom logger
 */
export const normalizeContentLengthHeader = (headers, body, options = {}) => {
  const logger = options.logger || defaultLogger
 ...
 */
export const createTransactionResponse = (response, body) => {
 ...
 * @param {Buffer} body
 */
export const detectBodyEncoding = (body) => {
