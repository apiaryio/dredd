      ...
      if (error.message) {
        process.stdout.write(
          `logger: ${JSON.stringify(
            logger,
            (key, value) => {
              // ES6 modules interoperability preserves the module's default export
              // under the "default" key. When attempted to stringify that,
              // the process gets into infinite loop due to circular JSON structure.
              // Explicitly ignoring the "default" key during the conversion to string.
              return key === 'default' ? null : value;
            },
            2
          )}`
        );
