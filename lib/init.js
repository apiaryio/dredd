...
function detectLanguage(files) {
  ...
  if (
    ['minil.toml', 'cpanfile', 'meta.json', 'build.pl'].some(f => lcFiles.includes(f))
  ) {
    return 'perl';
  }
  if (
    ['setup.py', 'requirements.txt', 'pipfile', 'pyproject.toml', 'setup.cfg', 'manifest.in'].some(f => lcFiles.includes(f))
  ) {
    return 'python';
  }
  if (
    ['gemfile', 'gemfile.lock'].some(f => lcFiles.includes(f))
    || lcFiles.some(f => f.match(/\.gemspec$/))
  ) {
    return 'ruby';
  }
...
init._printClosingMessage = printClosingMessage;
init._detectLanguage = detectLanguage;
init._detectServer = detectServer;
init._detectApiDescription = detectApiDescription;
init._detectCI = detectCI;
init._updateAppVeyor = updateAppVeyor;
